...
function shouldShowSessionsTable(provider = getCurrentProvider()) {
  return provider !== 'both' && provider !== 'cursor' && provider !== 'opencode' && provider !== 'api-integrations';
}
...
function quotaOrderForProvider(provider) {
  ...
  if (provider === 'cursor' || provider === 'opencode') return cursorQuotaOrder;
...
const renewalCategories = {
  ...
  gemini: [],
  opencode: opencodeChartColorFallback,
  cursor: [
...
async function fetchCycles() {
  ...
  const provider = requestProvider;
  const loggingHistoryProviders = new Set(['synthetic', 'zai', 'anthropic', 'copilot', 'codex', 'antigravity', 'minimax', 'gemini', 'cursor', 'opencode']);
...
function renderCyclesTable() {
  ...
  const usePercent = provider === 'anthropic' || provider === 'copilot' || provider === 'codex' || provider === 'antigravity' || provider === 'minimax' || provider === 'gemini' || provider === 'openrouter' || provider === 'cursor' || provider === 'opencode';
  ...
  if (pageData.length === 0) {
    const emptyMsg = isLoggingHistory
      ? (provider === 'cursor' || provider === 'opencode' ? 'No Cursor usage samples in this range.' : 'No logging data in this range.')
      : (provider === 'cursor' || provider === 'opencode' ? 'No Cursor billing-cycle samples in this range.' : 'No polling data in this range.');
...
function addOverrideRow(quotaKey, provider, warning, critical, isAbsolute, disableReset, disableWarning, disableCrit) {
  ...
  row.innerHTML = `
    <select class="settings-input override-provider-select" style="flex:1">
      ...
      <option value="cursor" ${provider === 'cursor' || provider === 'opencode' ? 'selected' : ''}>Cursor</option>
